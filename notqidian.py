...
def main():
    ...
    print("(形如: https://www.uukanshu.com/b/69778/)")
    novel_url = input("请输入: ")
    r = s.get(novel_url)
    # print("==========================")
    # print(r.content)
    ...
    _jieshao = soup.find("dd", {"class": "jieshao-content"})
    # print(f"jieshao {_jieshao}")
    # print(f"jieshoa {_jieshao.get_text()}")
    # book_info = _jieshao.find("h3")
    # book_title = book_info.find('em').string
    book_title = book_cover_src = soup.find('a', {'class': 'bookImg'}).find('img')['alt']
    # book_author = book_info.find('a').string
    # _jieshao_h2 = _jieshao.find('h2')
    # book_author = _jieshao_h2.find("a").get_text()
    book_author = input("what is book author: ")
    # get all
